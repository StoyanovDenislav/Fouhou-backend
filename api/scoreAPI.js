...
/**
 * DELETE /truncate/:gameID
 * Truncate game scores
 */
router.delete('/truncate/:gameID', async (req, res) => {
  try {
    const { gameID } = req.params;
    const db = getDB();
    
    console.log(`🗑️ Truncating all records for game: ${gameID}`);
    ...
    console.log(`✅ Deleted ${countBefore[0]?.count || 0} records for game ${gameID}`);
    
    res.json({
      success: true,
      message: `All records deleted for game ${gameID}`,
      gameID,
      deletedCount: countBefore[0]?.count || 0,
      timestamp: new Date().toISOString()
    });
    
  } catch (error) {
    handleError(res, error, 'Failed to truncate game records');
  }
});
 ...
 * Truncate all scores
 */
router.delete('/truncate-all', async (req, res) => {
  try {
    const db = getDB();
    
    console.log(`🗑️ TRUNCATING ALL RECORDS from GameScores table`);
    
    const countBeforeQuery = `SELECT COUNT(*) as count FROM GameScores`;
    const countBefore = await db.query(countBeforeQuery);
    
    const truncateQuery = `TRUNCATE CLASS GameScores`;
    await db.query(truncateQuery);
    
    console.log(`✅ Truncated ${countBefore[0]?.count || 0} records from GameScores`);
    
    res.json({
      success: true,
      message: 'All records deleted from GameScores',
      recordsDeleted: countBefore[0]?.count || 0,
      timestamp: new Date().toISOString()
    });
    
  } catch (error) {
    handleError(res, error, 'Failed to truncate all records');
  }
});
